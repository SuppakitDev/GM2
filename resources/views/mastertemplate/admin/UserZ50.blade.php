...
@endif 
              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  ...
                  <a href="#adduserz50" data-toggle="modal"><button type='button' class='btn btn-success'><i class="fa fa-plus"></i> Add manager</button></a>
                  <div class="x_content">
                    <table class="table table-striped table-bordered">
                      ...
                      @foreach($userZ50 as $userZ50s)
                 
                      <tbody>
                        <tr>
                          <td class="priority-1" >{{$userZ50s->Fname}}</td>
                          <td class="priority-2" >{{$userZ50s->Lname}}</td>
                          <td class="priority-3" >{{$userZ50s->Tel}}</td>
                          <td class="priority-4" >{{$userZ50s->email}}</td>
                          <td class="priority-5" style="text-align:center;">
                          <a href="#edituserZ50{{$userZ50s->id}}" data-toggle="modal"><button type='button' class='btn btn-warning'><i class="fa fa-pencil-square-o"></i></button></a>
                          </td>
                          <td class="priority-6" style="text-align:center;">
                          <?= Form::open(array('url' => 'Userz50/'.$userZ50s->id,'method'=>'DELETE')) ?>
                        ...
                        <!-- MODAL Edit -->
                            <div id="edituserZ50{{$userZ50s->id}}" class="modal fade" role="dialog" >
                                <div class="w3-center"><br>
                                    <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:500px;margin-top:10%">
                                    <?= Form::model($userZ50s, array('url'=> 'adminprofile/'.$userZ50s->id, 'method' => 'PUT','files' => true)) ?>
                                        {{ csrf_field() }}
                                        <div class="modal-content">
                                        <div class="modal-body">
                                        <div class="col-xs-12"  align='center' style="padding-bottom: 15px;" >
                                            <img src="img/profiles/resize/{{$userZ50s->image}}" class="img-circle" alt="User Image">
                                        </div>
                                        <div class="col-xs-6" >
                                            <div class="form-group" >
                                                <?= Form::label('Fname','First Name');?>
                                                <?= Form::text('Fname',$userZ50s->Fname,['class' => 'form-control select2','placeholder' => 'First Name' ,'style' => 'width: 100%;' ]);?>
                                            </div>
                                        </div>
                                        <div class="col-xs-6" >
                                            <div class="form-group" >
                                                <?= Form::label('Lname','Last Name');?>
                                                <?= Form::text('Lname',$userZ50s->Lname,['class' => 'form-control select2','placeholder' => 'Last Name']);?>
                                            </div>
                                        </div>
                                        <div class="col-xs-6" >
                                            <div class="form-group" >
                                                <?= Form::label('username','UserName');?>
                                                <?= Form::text('username',$userZ50s->username,['class' => 'form-control','placeholder' => 'UserName']);?>
                                            </div>
                                        </div>
                                        <div class="col-xs-6" >
                                            <div class="form-group" >
                                                <?= Form::label('Tel','Phone Number');?>
                                                <?= Form::text('Tel',$userZ50s->Tel,['class' => 'form-control','placeholder' => 'Phone Number']);?>
                                            </div>
                                        </div>
                                        <div class="col-xs-6" >
                                            <div class="form-group" >
                                                <?= Form::label('email','E-mail');?>
                                                <?= Form::text('email',$userZ50s->email,['class' => 'form-control','placeholder' => 'E-mail']);?>
                                            </div>
                                        </div>
                                        <?= Form::hidden('image55',$userZ50s->image,['class' => 'form-control']);?>
                    ...
                    {!! Form::close() !!}
                        ...
                        <!-- MODAL ADD -->
                        <div id="adduserz50" class="modal fade" role="dialog" >
                                ...
                                <div class="form-group{{ $errors->has('site') ? ' has-error' : '' }}">
                                    <div class="col-md-12">
                                        ...
                                        <option value="{{ old('site') }}" required>{{ old('site') }}</option>     
                                            @foreach($Sitez50 as $Sitez50s)
                                                <option value="{{$Sitez50s->Site_ID}}">{{$Sitez50s->SiteName}}</option>                                                
                                            @endforeach    
                                            </select>
                                        @if ($errors->has('site'))
                                        <span class="help-block">
